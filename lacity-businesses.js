...
export async function handler() {
  try {
    ...
    const items = (data || []).map((row, i) => ({
      ...
      apiSource: 'lacity', source: 'LA City Businesses',
      location: [row.business_address, row.city, row.zip_code].filter(Boolean).join(', ') || 'Los Angeles, CA',
      triggers: ['business_formation', 'new_registration', 'public_record'], consentStatus: 'business_public_record', behaviors: ['new_opening'],
